...
%>
<c:set var="tm" value="<%=ManagementFactory.getThreadMXBean()%>" />
<table class="table table-border table-bordered table-bg mt-20">
	<thead>
		<tr>
			<th colspan="2" scope="col">JVM Thread Info</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>Thread Count</td>
			<td>${tm.threadCount}</td>
		</tr>
	</tbody>
</table>
<%-- 
...
<%
	...
	for (int i = 0; i < tia.length; i++) {
		...
		long cpuTime = tm.getThreadCpuTime(tia[i].getThreadId())
				/ (1000 * 1000 * 1000);
	...
	for (int t = 0; t < threadArray.length; t++) {
		ThreadInfo ti = tm.getThreadInfo(threadArray[t][0],
				Integer.MAX_VALUE);
		if (ti == null)
			continue;
%>
